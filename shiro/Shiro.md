...
在config包下创建一个ShiroConfig配置类

使用@Configuration和@Bean将配置类加入容器中

> 需要配置的
>
> 1. ShiroFilterFactoryBean 总体配置类，配置安全管理器、公共/受限资源、配置默认界面
> 2. SecurityManager 安全管理器，这里使用其子类DefaultWebSecurityManager
> 3. Realm 领域类，设置认证和授权的配置
...
#### 配置ShiroFilterFactoryBean 
    ...
    public ShiroFilterFactoryBean shiroFilterFactoryBean(SecurityManager manager){
        ...
        //配置默认认证界面,即无权访问受限资源时会自动跳转这个未登录请求
...
> Realm类用于编写具体认证和授权的流程

```java
//在config中已经使用@Bean将此类加入了容器，所以不需要使用@Component注解
public class CustomerRealm extends AuthorizingRealm {
    @Autowired
