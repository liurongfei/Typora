...
## 4.Netty实现WebSocket服务端

### 4.1 什么是WebSocket

> WebSocket 是一种标准协议，用于在客户端和服务端之间进行双向数据传输。它是一种基于 `TCP`的一种独立实现，跟 HTTP 没什么关系，但是复用了HTTP的握手通道。
...
由客户端发起协议升级请求,采用的是标准的HTTP报文格式，且只支持`GET`方法,
...
* `Connection: Upgrade`:表示该连接需要升级协议
* `Upgrade: websocket `:表示该连接要升级到websocket协议
* `Sec-WebSocket-Version: 13`:表示需要升级到的版本
* `Sec-WebSocket-Key`:与后面服务端响应首部的`Sec-WebSocket-Accept`是配套的，提供基本的防护，比如恶意的连接，或者无意的连接。

> 注意，上面请求省略了部分非重点请求首部。由于是标准的HTTP请求，类似Host、Origin、Cookie等请求首部会照常发送。在握手阶段，可以通过相关请求首部进行 安全限制、权限校验等。
>
> 该HTTP请求对应netty的`HttpRequest`
